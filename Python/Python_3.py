...
if currency_convertor:
    ...
    currency_arr = ['eur', 'uah', 'chf', 'rub', 'byn']
    print('Возможная валюта для обмена:', currency_arr)
    target_currency = input('Выберите валюту, которую хотите конвертировать (из приведенного списка, без пробела):')
    try:
        target_currency_amount = int(input('Введите количество валюты:'))
    except ValueError:
        print('Введите числовое значение!')
        try:
            target_currency_amount = int(input('Введите количество валюты:'))
        except ValueError:
            target_currency_amount = float(input('Повторите пожалуйста ввод (количество валюты):'))
    currency_result = 0
